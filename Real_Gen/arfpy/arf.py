...
class arf:
  ...
  :param epochs: Maximum iterations for the adversarial loop, defaults to 10
  ...
  :rtype: dict
  """   
  def __init__(self, num_trees = 30, delta = 0, epochs =10, early_stop = True, verbose = True, min_node_size = 5, dist = "truncnorm", oob = False, alpha = 0):

    # assertions
    assert epochs >= 0, f"negative number of iterations is not allowed: parameter epochs must be >= 0"
  ...
  def fit(self, x, discrete_columns=(), epochs=None):
    ...
    clf_0 = RandomForestClassifier( oob_score= True, n_estimators=self.num_trees,min_samples_leaf=self.min_node_size) 
    clf_0.fit(x, y)
    ...
    if (acc_0 > 0.5 + self.delta and iters < epochs):
      converged = False
      while (not converged): # Start adversarial loop
        ...
        # discrimintator
        clf_1 = RandomForestClassifier( oob_score= True, n_estimators=self.num_trees, min_samples_leaf=self.min_node_size) 
        clf_1.fit(x, y)
        ...
        iters = iters + 1
        plateau = True if self.early_stop is True and acc[iters] > acc[iters - 1] else False
        # if self.verbose is True:
        #   print(f"Iteration number {iters} reached accuracy of {acc_1}.")
        if (acc_1 <= 0.5 + self.delta or iters >= epochs or plateau):
          converged = True
        else:
          clf_0 = clf_1
    self.clf = clf_0
    ...
    for tree_num in range(0, self.num_trees):
      ...
      to_prune = unique[counts < self.min_node_size]
